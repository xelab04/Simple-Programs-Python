...
def directory_n_exist():
    ...
    text.pack()
    button = tk.Button(new_window,text="Proceed",height=2,width=20,command=lambda: dirs(new_window))
    button.pack()
...
def view_license():
    try:
        ...
        subprocess.run(['open', license_dir], check=True)
    except:
        pass
...
def helps():
    ...
    try:
        ...
        print(help_dir)
        subprocess.run(['open', help_dir], check=True)
    except:
        pass
